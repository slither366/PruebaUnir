...
public class DlgADMTrazabilidad extends JDialog {
    ...
    private JMenuItem itemLiberarPaciente;
    ...
    private FarmaTableModel modelTblPacientesClone;
    private FarmaTableModel modelTblPacientesFiltradoClone;
    ...
    private JButton btnVerLiberados = new JButton();

    public DlgADMTrazabilidad() {
        this(null, "", false, "");
    }

    public DlgADMTrazabilidad(Frame parent, String title, boolean modal, String codMedico) {
        
        super(parent, title, modal);
        myParentFrame = parent;
        try {
            this.varCodMedico = codMedico;
            jbInit();
            initialize();
            InicializarMenu();
        } catch (Exception e) {
    ...
    public void InicializarMenu(){
        ...
        itemLiberarPaciente = new JMenuItem("Liberar Atención");
        itemLiberarPaciente.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                liberarOrdenPaciente();
            }
        });
        popup.add(itemLiberarPaciente);      
    }
    ...
    private void jbInit() throws Exception {
        ...
        this.setTitle("Seguimiento de Consultas en Tiempo Real");
        
        pnlContenedor.setBounds(new Rectangle(0, 0, 675, 410));
        // TABLA DE LISTA
        ...
        //Dflores: 16/08/2019 Redimensionado
        scpPacientes.setBounds(new Rectangle(10, 90, 1075, 285));
        ...
        btnVerLiberados.setText("Ver Liberadas");
        btnVerLiberados.setBounds(new Rectangle(25, 15, 105, 30));
        btnVerLiberados.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    btnVerLiberados_actionPerformed(e);
                }
            });
        jPanelHeader1.add(btnVerLiberados, null);
        ...
        tblPacientes.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                ...
                public void mouseReleased(MouseEvent e) {
                    tblPacientes_mouseReleased(e);
                }
            });
    }
    //Dflores: 27/08/2019
    public void liberarOrdenPaciente(){
        int selecRow = getNumFilaSeleccionadaTabla();
        int pos =  tblPacientes.getSelectedRow();
        //String pCodPaciente = FarmaUtility.getValueFieldJTable(myJTable, tblPacientes.getSelectedRow(), COL_COD);
        String pNroAtencion = FarmaUtility.getValueFieldArrayList(modelTblPacientes.data, selecRow, 17);
        String pEstado = FarmaUtility.getValueFieldArrayList(modelTblPacientes.data, selecRow, 5);
        
        if(isUserAdministradorLocal()){
            if(validaEstado(pEstado)){
                /*FarmaUtility.showMessage(this, "Se Liberará la Atención!",txtEspecialidad);

                JInputDialog df = new JInputDialog(frame1,"Ingrese el Motivo de LIBERACION:",false);
                //String dato = JOptionPane.showInputDialog(this, "Ingrese parentesco:", "");
                if(df!=null){
                    String motLibera = JInputDialog.showInputDialog(null,"Ingrese el Motivo de LIBERACION:","AF");
                    //liberarAtencionMedica(pNroAtencion,"010",motLibera);
                    FarmaUtility.showMessage(this, "Se realizó la Liberacion!",txtEspecialidad);
                }else{
                    FarmaUtility.showMessage(this, "Canceló! No se liberó... ",txtEspecialidad);
                }*/
            
                accionLiberar(pNroAtencion,selecRow);

            }else{
                FarmaUtility.showMessage(this, "No se puede Liberar un Estado: " + pEstado,txtEspecialidad);
            }
        }else{
            //vAutorizaCambioPrecio = false;
            //txtPrecioFinal.setEnabled(false);
            FarmaUtility.showMessage(this, "Usuario No autorizado para Liberar Atención! \n" +
                "Comuniquese con el Administrador!",txtEspecialidad);
            //FarmaUtility.moveFocus(txtCantidad);
        }
    /*
        if(tblPacientes.getSelectedRow()>=0){
            String pCodProd = FarmaUtility.getValueFieldJTable(myJTable, tblPacientes.getSelectedRow(), COL_COD);
            try {
    ...
    private void btnBuscar_actionPerformed(ActionEvent e) {
        if (validarFechas()) {
            ...
            valTodos = modelTblPacientes.data.size();
    ...
    private void initialize(){
        crearTablePaciente();
        //Dflores: data para tabla especialidad
        //if(this.varCodMedico.equals("")){
    ...
    private void cargarDatosEspecialidad(){
        ...
        muestraLaboratorios(this.varCodMedico, VariablesCentroMedico.vVarFiltroListaAtenciones);
    }
    //Dflores: 16/08/2019
    ...
    private void crearTablePaciente(){
        ...
        FarmaColumnData[] columnasListaEspera = { //Dflores: 16/08/2019 Redimensionado de campos
                                                  new FarmaColumnData("Fecha", 70, JLabel.CENTER),       //0
                                                  new FarmaColumnData("Hora", 60, JLabel.CENTER),       //1
        ...
        modelTblPacientesClone = new FarmaTableModel(columnasListaEspera, UtilityPtoVenta.obtenerDefaultValuesTabla(columnasListaEspera.length),0);
        modelTblPacientesFiltradoClone = new FarmaTableModel(columnasListaEspera, UtilityPtoVenta.obtenerDefaultValuesTabla(columnasListaEspera.length),0);
    ...
    private void cargaListaTrazabilidad(String fecIni, String fecFin) {
        try {
            ...
            modelTblPacientesClone.data = modelTblPacientes.data;
            //modelTblPacientes.data = modelTblPacientesFiltradoClone.data;
    ...
    private void filtroProdEspecialidad() {
        ...
        if(!condicion.equals("") && condicion.length() > 0){
            ...
            //filtrar java
            ArrayList target = modelTblPacientes.data;//tableModelListaPrecioProductos.data;        
            ArrayList filteredCollection = new ArrayList();
            
            Iterator iterator = target.iterator();
            while(iterator.hasNext()){
                ArrayList fila = (ArrayList)iterator.next();
                String descProd = fila.get(20).toString().toUpperCase().trim();
                if(descProd.equals(condicion)){
                    filteredCollection.add(fila);
                }
            }
                     
            modelTblPacientes.data = filteredCollection;
            modelTblPacientes.fireTableDataChanged();
            ...
            if(tblPacientes.getRowCount()==0){
                ...
                if(tblPacientes.getRowCount()==1)
                    lblMensajeFiltro.setText(tblPacientes.getRowCount()+" fila para el filtro aplicado");
                else
                    lblMensajeFiltro.setText(tblPacientes.getRowCount()+" filas para el filtro aplicado");
            }
            ...
            actualizarCantSemaforos();
            pintaListaxFiltro();
            modelTblPacientesFiltradoClone.data = modelTblPacientes.data;
        }else{
            clonarListadoProductos(true);
            //lblMensajeFiltro.setText("No se encontraron datos para el filtro ingresado");
            //FarmaUtility.showMessage(this, "No se encontraron datos para la siguiente busqueda!", null);
        ...
        if(tblPacientes.getRowCount()>0)
            FarmaGridUtils.showCell(tblPacientes, 0, 0);
            actualizarCantSemaforos();
            pintaListaxFiltro();
            modelTblPacientesFiltradoClone.data = modelTblPacientes.data;
    }
    ...
    private void clonarListadoProductos(Boolean reset) {
        if(reset){
            ...
            for (int i = 0; i < modelTblPacientesClone.data.size(); i++) { 
                ArrayList aux = (ArrayList)((ArrayList)modelTblPacientesClone.data.get(i)).clone();
                arrayClone.add(aux);
            }
            modelTblPacientes.clearTable();
            modelTblPacientes.data = arrayClone;
            tblPacientes.repaint();
            tblPacientes.show();
        }else{
            //MODIFICAR EN ESTE PUNTO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! REVISANDO
            ArrayList arrayClone = new ArrayList();
            for (int i = 0; i < modelTblPacientes.data.size(); i++) { 
                ArrayList aux = (ArrayList)((ArrayList)modelTblPacientes.data.get(i)).clone();
                arrayClone.add(aux);
            }
            modelTblPacientes.clearTable();
            modelTblPacientes.data = arrayClone;
    ...
    private void clonarPacientesxSemaforo(String semaforo) {
        ...
        if(semaforo.equals("VER TODOS")){
            ...
            for (int i = 0; i < modelTblPacientesFiltradoClone.data.size(); i++) { 
                String estadoConsulta = FarmaUtility.getValueFieldArrayList(modelTblPacientesFiltradoClone.data, i, 5);
                
                if(estadoConsulta.equals(semaforo)){
                    ArrayList aux = (ArrayList)((ArrayList)modelTblPacientesFiltradoClone.data.get(i)).clone();
                    arrayClone.add(aux);
                }
            }
            modelTblPacientes.clearTable();
    ...
    private void actualizarCantSemaforos(){
        ...
        lblSemaforo6.setText(tblPacientes.getRowCount()+"");
    }
    ...
    private void contarTipoConsulta() {
        ...
        ArrayList target = modelTblPacientes.data;//tableModelListaPrecioProductos.data;        
        Iterator iterator = target.iterator();
        valTodos = modelTblPacientesFiltradoClone.data.size();
        
        while(iterator.hasNext()){
            ArrayList fila = (ArrayList)iterator.next();
            String estadoConsulta = fila.get(5).toString().toUpperCase().trim();
    ...
    private void tblPacientes_mouseReleased(MouseEvent e) {
        int r = tblPacientes.rowAtPoint(e.getPoint());
                   if (r >= 0 && r < tblPacientes.getRowCount())
                       tblPacientes.setRowSelectionInterval(r, r);
                   
           //  jPopupMenu.show(evt.getComponent(), evt.getX(), evt.getY());                
    }

    private void btnVerLiberados_actionPerformed(ActionEvent e) {
        DlgAtencionesLiberadas dlgAtencionesLiberadas = new DlgAtencionesLiberadas(frame1, "", true);
        dlgAtencionesLiberadas.setVisible(true);
    }
    ...
    class CustomRenderer extends DefaultTableCellRenderer 
    {
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column)
        {
            ...
            if(modelTblPacientes.data.size()>0){
                String  valEstado=tblPacientes.getValueAt(row, column).toString().trim();
    ...
    public void pintaListaxFiltro(){
        if (tblPacientes.getRowCount() > 0) {
            tblPacientes.getColumnModel().getColumn(5).setCellRenderer(new CustomRenderer()); 
        }
