...
public class DlgListaMedicoEspera extends JDialog {

    private static final Logger log = LoggerFactory.getLogger(DlgListaMedicoEspera.class);
    ...
    public JLabel txtMedicoPaciente = new JLabel();
    ...
    public DlgListaMedicoEspera() {
        this(null, "", false, null);
    }

    public DlgListaMedicoEspera(Frame parent, String title, boolean modal, TipoAtencionCM tipoLista) {
    ...
    private void jbInit() throws Exception {
        ...
        this.setSize(new Dimension(997, 525));
        ...
        lblHCFisica.setBounds(new Rectangle(15, 30, 85, 20));
        ...
        lblEspecialidadPaciente.setBounds(new Rectangle(1050, 85, 85, 20));
        ...
        jLabel1.setBounds(new Rectangle(1055, 120, 90, 15));
        jLabel1.setFont(new Font("SansSerif", 1, 11));
        jLabel1.setForeground(new Color(0, 114, 169));
        cmbConsultorio.setBounds(new Rectangle(1145, 115, 225, 20));
        ...
        jButton1.setBounds(new Rectangle(565, 10, 105, 20));
        ...
        lblEtiquetaFecha.setBounds(new Rectangle(40, 55, 40, 20));
        lblEtiquetaFecha.setFont(new Font("SansSerif", 1, 12));
        lblEtiquetaFecha.setForeground(new Color(0, 74, 115));
        lblFecha.setBounds(new Rectangle(85, 55, 130, 20));
        ...
        pnlTituloTablaPacientes.setBounds(new Rectangle(5, 80, 960, 20));
        tblPacientes.setFont(new Font("SansSerif", 0, 11));
        scpPacientes.setBounds(new Rectangle(5, 100, 960, 330));
        scpPacientes.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS);

        // TECLAS DE FUNCION

        lblEsc.setBounds(new Rectangle(5, 445, 117, 19));
        lblEsc.setName("ESC");
        lblEsc.setText("[ ESC ] Salir");

        lblF1.setBounds(new Rectangle(135, 445, 117, 19));
        lblF1.setName("F1");
        lblF1.setText("[ F1 ] Anular");


        lblF5.setBounds(new Rectangle(260, 445, 117, 19));
        lblF5.setName("F5");
        lblF5.setText("[ F5 ] Actualizar");

        lblF11.setBounds(new Rectangle(665, 445, 117, 19));
        lblF11.setName("F11");
    ...
    private void this_windowOpened(WindowEvent e) {
        ...
        //cargarDatosPantalla();
        int filas = modelTblPacientes.getRowCount();
        if(filas>0){
            FarmaUtility.moveFocusJTable(tblPacientes);
            mostrarTitulo();
        }else{
            FarmaUtility.moveFocus(jButton1);
        }
    }
    ...
    private void cargarTablaLista(){
        ...
        String pConsultorio = "0";//FarmaLoadCVL.getCVLCode(cmbEspecialidad.getName(), cmbEspecialidad.getSelectedIndex());
        String pBus = "0";//FarmaLoadCVL.getCVLCode(cmbConsultorio.getName(), cmbConsultorio.getSelectedIndex());
    ...
    private void funcionF11(MouseEvent e) {
    }
    ...
    private void jButton1_keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
            ...
            int filas = modelTblPacientes.getRowCount();
            if(filas>0){
                FarmaUtility.moveFocus(tblPacientes);
                tblPacientes.changeSelection(0, 0, false, false); 
                mostrarTitulo();
            }else{
                //System.out.println("No existen filas en la tabla tblPacientes");    
                FarmaUtility.moveFocus(jButton1);
            }
        } else if (e.getKeyCode() == KeyEvent.VK_ENTER){
            cargarDatosPantalla();   
