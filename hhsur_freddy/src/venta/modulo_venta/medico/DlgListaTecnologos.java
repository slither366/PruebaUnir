...
public class DlgListaTecnologos extends JDialog 
{
    private static final Logger log = LoggerFactory.getLogger(DlgListaTecnologos.class);  
...
private boolean vTipoDocAsociado = false;
  public DlgListaTecnologos()
  {
    this(null, "", false,false);
  }

  public DlgListaTecnologos(Frame parent, String title, boolean modal,boolean vTipoDocAsociado)
  {
  ...
  private void jbInit() throws Exception
  {
    ...
    this.setTitle("Lista de Tecnologos");
    ...
    btnNombre.setText("<html><center>Nombre \u00f3 C\u00f3digo :</center></html>");
    ...
    btnRelacion.setText("Relacion de Tecnologos ");
  ...
  {
    tableModelListaTecnologos = new FarmaTableModel(ConstantsMedico.columnsListaTecnologos, ConstantsMedico.defaultValuesListaTecnologos,0);
    FarmaUtility.initSimpleList(tblMedicos,tableModelListaTecnologos, ConstantsMedico.columnsListaTecnologos);
  }
  ...
  private void cargaListaTecnicos()
  {
    try
    {
            DBMedico.listaTodosTecnologos(tableModelListaTecnologos);
  ...
  private void txtNombreMedico_keyPressed(KeyEvent e)
  {
    ...
    private void seleccionaMedico() {
       ...
       FarmaUtility.showMessage(this, "Por favor de seleccionar una fila.", txtNombreMedico);
           ...
           if(vTipoDocAsociado){
               VariablesModuloVentas.VNUM_DNI_TECNOLOGO_IN = FarmaUtility.getValueFieldArrayList(tableModelListaTecnologos.data,
                                                                                pos,
                                                                                0).trim();
               VariablesModuloVentas.VDATOS_TECNOLOGO_IN= FarmaUtility.getValueFieldArrayList(tableModelListaTecnologos.data,
                                                                                pos,
                                                                                1).trim();
               
           }
           else{
               VariablesModuloVentas.VNUM_CMP_IN = FarmaUtility.getValueFieldArrayList(tableModelListaTecnologos.data,
                                                                                pos,
                                                                                0).trim();
               VariablesModuloVentas.VDATOS_MEDICO_IN= FarmaUtility.getValueFieldArrayList(tableModelListaTecnologos.data,
                                                                                pos,
                                                                                1).trim();
    ...
    private void filtrar() {
        ...
        if(pCadena.length()==0)
            ...
            try
            {
                DBMedico.listaBusquedaTecnologos(tableModelListaTecnologos, pCadena);
    ...
    private void creaMedico() {
        DlgAdmTecnologos dlgIngMedico = new DlgAdmTecnologos(myParentFrame, "", true);
    ...
    private void busquedaMedico() {
        ...
        try {
                DBMedico.getTecnologoDNI(vLista,codCMP.trim());
